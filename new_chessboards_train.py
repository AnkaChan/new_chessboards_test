# -*- coding: utf-8 -*-
"""
Created on Tue Apr 11 20:42:32 2017
...
#模型保存的路径及文件名
MODEL_SAVE_PATH = "p0chessboards_model"     # 模型保存路径
MODEL_NAME = "p0chessboards_model"          # 模型名称
...
# 定义代价函数yi
    ...
    '''
    loss = tf.reduce_mean(tf.where(tf.greater(y_, a), 
                                  tf.square(tf.maximum(y_-y, 0))*76800/49, 
                                  tf.square(tf.maximum(y,0))*76800/76751))
    loss = -tf.reduce_sum(tf.where(tf.greater(y_, a), 
                                   (y_*tf.log(tf.clip_by_value(y, 1e-6, 1.0)))/(49*1400), 
                                   (tf.log(1-tf.clip_by_value(y, 0, 0.999999)))/(76751*1400))) 

    loss = -tf.reduce_mean(tf.where(tf.greater(y_, a), 
                                   (y_*tf.log(tf.clip_by_value(y, 1e-6, 1.0)))*(76800/49), 
                                   (tf.log(1-tf.clip_by_value(y, 0, 0.999999)))*(76800/76751)))
    '''
    loss = -tf.reduce_sum(tf.where(tf.greater(y_, a), 
...
# 初始化TensorFlow持久化类。
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            #print(np.sum(ys))
        np.savetxt('cost_value.txt',listrr)
